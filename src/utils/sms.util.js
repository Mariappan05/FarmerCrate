...
const sendSMS = async (mobileNumber, message) => {
  try {
    ...
    // Try to load Twilio module
    let twilio;
    try {
      twilio = require('twilio');
    } catch (moduleError) {
      console.warn('Twilio module not found. Using mock SMS for development.');
      console.log(`[MOCK SMS] To: ${mobileNumber}, Message: ${message}`);
      return true; // Return true for development
    }
    ...
    // For development, return true even if SMS fails
    console.warn('SMS failed, but continuing with mock mode for development');
    console.log(`[MOCK SMS] To: ${mobileNumber}, Message: ${message}`);
    return true;
  }
};
